...
"""Launch the Nemotron SAE text dashboard locally.

Copies the dashboard data produced by ``scripts/dashboard.py`` into the
``atlas/`` app's ``public/`` directory and starts the Vite dev server.

Usage:
    # After generating data (or scp'ing it from a GPU box):
    python scripts/dashboard.py --checkpoint ... --output-dir ./dash
...
def _get_live_feature_ids(data_dir: Path):
    """Return set of feature_ids with activation_freq > 0 (or None if no metadata)."""
    ...
    return set(df.loc[df["activation_freq"] > 0, "feature_id"].tolist())
...
def main():  # noqa: D103
    p = argparse.ArgumentParser(description="Launch Nemotron SAE text dashboard")
    p.add_argument(
        "--data-dir",
        type=str,
        required=True,
        help="Directory with features_atlas.parquet, feature_metadata.parquet, feature_examples.parquet",
    )
    p.add_argument("--port", type=int, default=5177)
    p.add_argument(
    ...
    dashboard_dir = Path(__file__).resolve().parent.parent / "atlas"
