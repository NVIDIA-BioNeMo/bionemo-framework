...
# Exit code /api/restart uses to ask a respawn supervisor for a fresh worker (distinct from a crash).
# Gated by ALLOW_ENGINE_RESTART=1, which the respawn wrapper sets.
EXIT_RESTART = 42
...
def health(response: Response):
    ...
    return {
        ...
        "restart_enabled": os.getenv("ALLOW_ENGINE_RESTART") == "1",  # UI shows the restart button iff true
...
def restart(response: Response):
    """Cancel the running work and reload the model — the UI's restart button.

    Exits the worker with EXIT_RESTART; a respawn wrapper (a `while :; do gpt2-sae-serve; done` loop, or
    systemd Restart=always) brings up a fresh one. Gated by ALLOW_ENGINE_RESTART=1 so it's a no-op
    unless the server is actually run under such a supervisor.
    """
    if os.getenv("ALLOW_ENGINE_RESTART") != "1":
        raise HTTPException(409, "restart not enabled — run under a respawn wrapper with ALLOW_ENGINE_RESTART=1")
    threading.Timer(0.3, lambda: os._exit(EXIT_RESTART)).start()  # let the 202 flush, then exit for respawn
