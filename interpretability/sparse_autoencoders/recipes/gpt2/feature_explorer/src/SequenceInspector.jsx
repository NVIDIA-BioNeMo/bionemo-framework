...
import { S, BASES_PER_LINE, Heat, Legend, resolveFeatureId, FeaturePicker, OrganismField, BackendBanner, RestartEngineButton, Row, Toggle, useUserLabels, userLabel } from './components'
...
const DEFAULT_SEQ = 'The cat sat quietly on the warm windowsill, watching the birds outside.'
...
export default function SequenceInspector() {
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={S.card}>
        <Row label="Text:">
          <div style={{ flex: 1 }}>
            <textarea value={sequence} onChange={(e) => setSequence(e.target.value)} rows={3}
              style={S.textarea} placeholder="Paste text…" />
            <div style={S.hint}>{cleaned.length} characters</div>
          </div>
        </Row>
        ...
        {mode === 'topk' ? (
          <Row label="Top-K:">
            ...
            <span style={S.help}>the K features that fire hardest on this text</span>
          </Row>
        ) : (
          <Row label="Features:">
      ...
      {!result ? (
        <div style={S.empty}>Paste text above and click <b>Annotate text</b> to see per-token SAE activations.</div>
      ) : (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      <div style={S.resultMeta}>
        {result.features.length} feature{result.features.length === 1 ? '' : 's'} · {bases.length} tokens ·
        layer {result.layer}
        {result.tag_len > 0 ? ` · ${result.tag_len} prefix token stripped` : ''}
      </div>
  ...
  return (
    <div style={S.featCard}>
      <div style={S.featHead}>
        <span style={S.featLabel}>{userLabel(feature.feature_id) || feature.label || `feature ${feature.feature_id}`}</span>
