...
const FeatureCard = forwardRef(function FeatureCard({ feature, isHighlighted, forceExpanded, onClick, loadExamples }, ref) {
  ...
  const [userTitle, setUserTitle] = useState('')  // in-progress edit buffer only; the shown name reads userLabel() live
  const inputRef = useRef(null)

  // Subscribe so this card re-renders when THIS or any feature is renamed in any tab/window. The card no
  // longer caches the stored title in state (displayTitle reads userLabel() live below), so a rename made
  // elsewhere while the card is mounted now updates it immediately instead of staying stale until remount.
  useUserLabels()

  // On entering edit mode, seed the buffer from the current stored name, then focus/select the input.
  useEffect(() => {
    if (editingTitle) {
  ...
  }, [editingTitle, feature.feature_id])
  ...
  const stored = userLabel(feature.feature_id)  // live read; re-runs each render (subscribed via useUserLabels)
  const displayTitle = editingTitle ? userTitle : (stored || description)
      ...
      <div style={styles.header} onClick={handleClick}>
        <div style={styles.headerLeft}>
          ...
          {editingTitle ? (
            ...
            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              <div style={stored ? styles.userTitle : styles.description}>{displayTitle}</div>
              <span
        ...
        <div style={styles.stats}>
          ...
          {/* v2 roadmap placeholder — populated when the eval pipeline lands. */}
        ...
        <div style={styles.expandedContent}>
          {feature.logo_path && (
            <div style={{ marginBottom: '14px' }}>
              <div style={styles.sectionHeader}>Token Context</div>
              <img
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <div style={styles.sectionHeader}>Top Activating Texts</div>
            <div style={styles.alignBar}>
              ...
              {['start', 'first_activation', 'max_activation'].map(mode => (
                ...
                >
                  {mode === 'start' ? 'text start' : mode === 'first_activation' ? 'first activation' : 'max activation'}
                </button>
              ))}
            </div>
          </div>
