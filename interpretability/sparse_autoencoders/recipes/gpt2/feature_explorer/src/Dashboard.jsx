...
const ALL_TABS = [
  {
    ...
    desc: 'Browse every feature in the SAE dictionary — its firing rate across the corpus, position in a decoder-space UMAP, top-activating example texts, and any label. Reads precomputed static files, so it needs no backend or GPU.',
    limits: [
      ...
      'Top examples come only from the precompute corpus — a feature may also fire on text not shown here.',
      'Labels are automatic (Neuronpedia) annotations and are incomplete; a handful of features are unlabeled.',
    ],
  },
  {
    id: 'steering', label: 'Generative steering', offline: false, // needs the live model
    desc: 'Generate text from a prompt while clamping chosen SAE features on the continuation, and compare against the unsteered baseline. Each request runs the live model end-to-end.',
    limits: [
      'A clamp sets an absolute SAE activation value (capped at ±300); large magnitudes can push the model off-distribution into repetitive or degenerate text.',
      'Steering affects only the generated continuation, at the SAE’s layer — the prompt itself is not rewritten.',
      'Prompt + generation must fit the 1024-token context: an over-length prompt is rejected, and the number of generated tokens is capped to what’s left after the prompt.',
  ...
  {
    id: 'inspector', label: 'Text inspector', offline: false, // needs live encode
    desc: 'Paste text to see per-token SAE feature activations — either the top-k features firing on it or specific feature ids you pick. Each request runs a live encode.',
    limits: [
      'Activations are read at the SAE’s single training layer only — not the whole network.',
      'The leading beginning-of-text token is shown but excluded from the top-k ranking.',
      'Text longer than the model’s 1024-token context is rejected, not truncated.',
      'Top-k ranks by each feature’s peak activation over the text, so broad-but-weak features can rank below sharp local ones.',
    ],
  },
  {
    id: 'sequmap', label: 'Text UMAP', offline: 'bundle', // offline iff dashboard.py embeddings bundle exists
    desc: 'Embed a set of texts (each pooled into one per-feature SAE vector), UMAP them to 2-D, then color or re-project the layout by any SAE feature. Uses the live backend, or a precomputed bundle when offline.',
    limits: [
      'UMAP is stochastic and only 2-D: the same texts can land in different spots between runs, and cluster sizes/distances aren’t quantitative.',
      'Each text is pooled (mean or max) over its tokens into one vector — within-text position is lost.',
      'Only features firing in at least min_firing texts are kept, so rare features drop out of the layout.',
      'Up to 1000 texts per request; texts past the cap are dropped, and those longer than the 1024-token context are clamped to it (embedded from their first 1024 tokens) — the tab reports the counts. The offline bundle is a fixed precomputed set.',
    ],
  },
]
...
export default function Dashboard() {
  ...
  return (
    <div style={S.shell}>
      <div style={S.tabBar}>
        <span style={S.brand}>GPT-2 SAE Feature Explorer</span>
        {TABS.map((t) => (
