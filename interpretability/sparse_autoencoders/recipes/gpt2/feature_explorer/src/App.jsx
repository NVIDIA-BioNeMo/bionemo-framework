...
import { userLabel, useUserLabels, hydrateUserLabels } from './components'
...
export default function App({ title = "GPT-2 SAE Feature Explorer", subtitle = "Real SAE features — GPT-2 small, layer 7", dark = true }) {
  useUserLabels() // re-render when a feature is renamed in any tab
  useEffect(() => { hydrateUserLabels() }, []) // pull backend-persisted renames once on load (shared across users)
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: 'var(--text)', lineHeight: '1.7' }}>
              ...
              <p style={{ margin: '0 0 16px' }}>
                The decoder logits histogram shows the projection of each feature's learned decoder weight vector through the language model's prediction head (unembedding), with the mean logit vector subtracted across all features. This mean-centering removes the model's shared baseline bias toward common tokens, so values reflect what each feature <em>specifically</em> promotes or suppresses relative to the average feature. Each bar represents a token. Green bars indicate tokens the feature promotes above baseline; red bars indicate tokens it suppresses below baseline. Gray bars have no feature-specific effect. This tells you what the feature pushes the model to output — not what activates it.
              </p>

              <h3 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '6px' }}>Top Activating Texts</h3>
              <p style={{ margin: '0 0 16px' }}>
                These are the text snippets where this feature fires most strongly. Each token is colored by its activation value — brighter highlights mean the feature responds more strongly at that position. This shows what <em>inputs trigger</em> the feature, which is conceptually distinct from decoder logits. A feature can activate strongly on a particular token or phrase without promoting that same token in the output — it may instead influence downstream or contextual predictions.
              </p>
