...
"""Generate the dashboard's static artifacts via three subcommands, split by cost.
...
def parse_args():
    """Parse the `atlas` / `examples` / `embeddings` subcommand and its options."""
    ap = argparse.ArgumentParser(description="Generate dashboard artifacts (atlas | examples | embeddings)")
    ...
    # embeddings — precompute the Sequence-UMAP bundle so that tab works with NO live backend.
    pm = sub.add_parser(
        "embeddings", help="sequmap_embeddings.json from a small FASTA (loads the 7B) — offline Sequence-UMAP"
    )
    _add_sae_args(pm)
    pm.add_argument("--evo2-ckpt-dir", default=os.environ.get("EVO2_CKPT_DIR"))
    pm.add_argument("--max-seq-len", type=int, default=int(os.environ.get("MAX_SEQ_LEN", "8192")))
    pm.add_argument("--examples-fasta", required=True, help="SMALL representative FASTA (bounded by sequence count)")
    pm.add_argument("--max-sequences", type=int, default=1000, help="Cap sequences read (keep it small)")
    pm.add_argument("--organism", default="None (raw DNA)", help="Phylo-tag preset to prepend")
    pm.add_argument("--batch-size", type=int, default=4)
    pm.add_argument(
        "--min-firing", type=int, default=1, help="Only keep feature_stats for features firing on >= N sequences"
    )
...
def _engine_and_seqs(args, min_len, what):
    """Load the full Evo2SAE engine + read --examples-fasta (shared by examples/embeddings; loads the 7B)."""
    ...
    print(f"[{what}] {len(seqs)} sequences, {eng.n_features} features, organism={args.organism!r}")
    return eng, ids, seqs, tag, (len(tag) if tag else 0)
...
def run_examples(args):
    ...
    eng, ids, seqs, tag, tag_len = _engine_and_seqs(args, 0, "examples")
...
def run_embeddings(args):
    """Precompute the Sequence-UMAP bundle as a STATIC JSON via the shared engine.embed_bundle().

    Byte-identical to the /gene_embed response, so the Sequence-UMAP tab reads this file and runs
    UMAP client-side with NO live backend. Bounded by sequence count.
    """
    import json

    eng, ids, seqs, tag, tag_len = _engine_and_seqs(args, 3, "embeddings")
    meta = [{"gene_symbol": sid, "label": None, "species": None} for sid in ids]
    bundle = eng.embed_bundle(
        [tag + s for s in seqs], tag_len, meta, min_firing=args.min_firing, batch_size=args.batch_size
    )
    if bundle is None:
        raise SystemExit("No encodable sequences")
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    dest = out / "sequmap_embeddings.json"
    dest.write_text(json.dumps(bundle))
    print(f"[embeddings] wrote {bundle['n_genes']} sequences x {bundle['n_features']} features -> {dest}")


def main():
    """Dispatch to the atlas / examples / embeddings subcommand."""
