...
import { BACKEND, fetchWithTimeout } from './backend'
...
export default function SequenceUMAPView({ height = 600 }) {
  ...
  async function embed() {
    ...
    try {
      ...
      const live = await fetchWithTimeout(`${BACKEND}/gene_embed`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ genes, organism }),
      }, 180000).catch(() => null)  // embedding many sequences is the slowest call; cap the wait
      let r
      if (live && live.ok) {
        r = await live.json()
      } else if (mode === 'preset') {
        const sb = await fetch(EMBEDDINGS_URL)
        if (!sb.ok) throw new Error('No live backend and no precomputed embeddings bundle found.')
        r = await sb.json() // precomputed full-library bundle (same shape as /gene_embed)
      } else {
      ...
      // The server now ships only the firing columns; feature_ids[col] -> real SAE feature id.
  ...
  async function saveLabel(fid, text) {
    ...
    try {
      const resp = await fetchWithTimeout(`${BACKEND}/label`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
