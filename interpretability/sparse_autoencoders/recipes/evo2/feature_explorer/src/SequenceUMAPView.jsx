  ...
  return (
    <div style={{ padding: 16, color: 'var(--text)' }}>
      ...
      {!bundle && initialLoading && (
        <div style={{ padding: 16, opacity: 0.7, fontSize: 13 }}>Loading sequence map…</div>
      )}
      {!bundle && !initialLoading && (
        <div style={{ border: '1px solid var(--border, #333)', borderRadius: 8, padding: 12, maxWidth: 720 }}>
          ...
          {backendReady === false && (
            <div style={{ marginTop: 8, fontSize: 12, color: '#f59e0b' }}>
              {mode === 'preset'
                ? '◐ Evo2 model + SAE loading… — Embed will serve the precomputed library until it’s ready.'
                : '◐ Evo2 model + SAE loading… (~1 min at startup) — pasting your own sequences needs the live model.'}
            </div>
          )}
          {(() => {
                ...
                return (
                  <div style={{ position: 'absolute', left: 8, right: 8, bottom: 8, background: 'var(--bg,#111)', border: '1px solid var(--border,#333)', borderRadius: 8, padding: '7px 9px', fontSize: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.45)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      {seq && <span style={{ opacity: 0.6, whiteSpace: 'nowrap' }}>· {seq.length} bp</span>}
