...
def test_long_context_preflight_requires_explicit_override_and_preserves_provenance(
    ...
    for requested, workload_tokens in ((50_000, 50_000), (300_012, 300_012)):
        profile = replace(profile_50k, max_model_len=requested)
        proof = profile_module.context_length_preflight(
            profile,
            model=tmp_path,
            workload_max_total_tokens=workload_tokens,
        )
        ...
        assert proof["workload_max_total_tokens"] == workload_tokens
        assert proof["workload_fits_resolved_max_model_len"] is True
        assert proof["workload_headroom_tokens"] == requested - workload_tokens
