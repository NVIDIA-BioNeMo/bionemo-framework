...
def test_vllm_tp2_matched_qc_profile_restores_all_production_objectives():
    config_path = RECIPE_ROOT / "configs" / "gdpo_phage_vllm_tp2_p8k12_full_qc.yaml"
    config = yaml.safe_load(config_path.read_text())
    env_config = config["env"]["phage_qc"]
    external_qc = env_config["external_qc"]
    mmseqs = env_config["mmseqs_cluster_diversity"]
    ...
    objective_names = [objective["name"] for objective in env_config["gdpo_objectives"]]
    checks = {
        "inherits qualified TP2 P8K12 route": config["defaults"]
        == "gdpo_phage_vllm_tp2_p8k12_full.yaml",
        "all production objectives": objective_names
        == [
            "valid_nt_chars",
        ...
        "MMseqs diversity enabled": mmseqs["enabled"] is True,
        "dustmask enabled": env_config["dustmask_filter"] is True,
        "separate matched-QC namespaces": "tp2" in config["checkpointing"]["checkpoint_dir"]
        and "full_qc" in config["checkpointing"]["checkpoint_dir"]
        and "tp2" in config["logger"]["log_dir"]
        and "full_qc" in external_qc["work_dir"]
        and "full_qc" in mmseqs["work_dir"],
    }
    failed = [name for name, passed in checks.items() if not passed]
    if failed:
        raise AssertionError(f"matched TP2 full-QC vLLM config drifted: {failed}")
