...
Put reusable project actions in planning/execution/scripts/. Put stage actions in STAGE/runs/ATTEMPT/scripts/. Record both in planning/execution/ACTIONS.yaml and copy the exact sanitized launch command into attempt command.sh. Runtime credentials use environment-variable references, approved runtime secret stores, or credential helpers; never inline tokens, signed URLs, cookies, passwords, or credential-bearing query parameters. Sanitize command.sh, scheduler/SSH wrappers, RUN.yaml, source_state.json, and resolved config copies before persistence, then scan the materialized bytes and reject writing, launch-record acceptance, or publication while any secret-bearing value remains. Record only the non-secret mechanism name and reproducible invocation structure.
...
Use set -Eeuo pipefail in Bash, quote expansions, absolute paths, no shell tracing around credentials, and no embedded secrets. Prefer atomic status writes. Never silently overwrite an attempt or duplicate a submission.
...
- Observe launch and the first one or two meaningful progress events at intervals justified by expected event timing and risk, while honoring site policy; examples include bytes, records, shards, batches, output files, evaluation units, generations, validation events, and checkpoints. Do not default to a 30-second heartbeat.
- After healthy progress is established, derive each source's wall-clock cadence from measured event timing and a useful fraction of the next progress or decision boundary. A stable long-running phase may use 5–30 minutes.
- Reset backoff on state change, new progress evidence, or health alert, but never tight-loop.
- Keep the recurring monitor scheduled while the facility reports active work; a background launch is not a completed handoff.
...
non-root runtime user and UID/GID, network policy, and explicit code/data/cache/result mounts; keep credentials out of the image and
