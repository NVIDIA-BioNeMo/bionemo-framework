...
Predeclare paired seeds, target-length generations, minimum cell size, uncertainty/equivalence rule, temperatures, prefix strata, fixed `top_k`/`top_p`, QC versions, and cell-completion evidence. Materialize those exact sampling values into every inference command and verify the command manifest matches the sweep contract; never silently hard-code the historical top-k 4/top-p 1 values into a different sweep. Span under-conditioned through copy-prone prefixes and low- through high-entropy temperatures; adapt the frontier instead of assuming the paper range. Keep training diagnostics separate: the current GRPO diagnostic inherits the no-top-k replay/reference policy (`top_k: null`) rather than the publication-era sampling sweep.

Follow the central resource policy to benchmark a bounded set of plausible TP/DP layouts at full length, or reuse a hash-compatible recent benchmark. Route long-running or unattended sweep launch and re-entry through bionemo-phage-design-adapt-execution. Choose the highest global valid-token throughput that fits and preserves semantics. Unless the user opted out, enable W&B in a calibration-specific project sharing the project-family prefix whenever the adapter's bounded supported authentication succeeds; if it does not, explicitly disable W&B in the attempt's resolved config, record the reason, and continue with authoritative local artifacts.

Before scoring, derive the required executables, databases, models, and outputs from the enabled objective/QC contract. Resolve external assets through the shared [asset-failover contract](../bionemo-phage-design/references/command-discovery.md#external-asset-failover). Pin their exact runtime paths and behavior-test the same scoring environment. Use the target phage as a positive control: every enabled score must be measurable, while its expected value/direction is reasoned from the objective (similarity should usually be strong; diversity or learned-model scores need not be 1). Include failure/no-signal controls. Treat an unexplained missing or fixed-zero metric as an environment/contract fault. Repair and rerun preflight; generation may continue, but selection may not.

Score raw and cluster-deduplicated hard-QC yield, lifecycle-wide target-host evidence, complete-genome integrity, key-gene/module and regulatory support, topology/packaging, composition envelopes, similarity to the approved viable-reference set, every enabled intended-use therapeutic component, invalidity, exact/circular matches to target or training data, reference identity, architecture/synteny change, 99%-cluster diversity, entropy, uncertainty, and filter completeness. A host-range model is one calibrated signal and does not replace these measurements. Report per-component reference and baseline support independently of hard-filter order. Treat unexplained missing or fixed-zero therapeutic components as preflight faults to diagnose; do not select settings by dropping them or weakening final QC. Cluster each cell independently when ranking settings; cross-cell clustering is only a proposed-mixture or final-order diagnostic.
