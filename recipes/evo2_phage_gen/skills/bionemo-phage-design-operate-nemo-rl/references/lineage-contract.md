...
Use the required schema-4 `sft_lineage` block from the objective and project contracts, populated
directly or through a verified immutable manifest reference that contains every required field. A
checkpoint path, general evidence list, or unconstrained selection mapping is not lineage. Keep
prompt lineage separate.
