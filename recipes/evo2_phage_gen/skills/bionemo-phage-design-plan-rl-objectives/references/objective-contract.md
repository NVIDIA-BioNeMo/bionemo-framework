...
therapeutic_quality:
  ...
  reward_support: []
sft_lineage:
  manifest_reference: null  # optional immutable manifest path/URI plus SHA-256 satisfying every field below
  manifest_sha256: null
  source_project: null
  source_result_root: null
  stage_name: null
  stage_type: null
  run_attempt_id: null
  checkpoint:
    ...
    provider_artifact_version: null
    content_sha256: null
    manifest_sha256: null
  ...
  training_data:
    ...
    leakage_controlled_split_manifest_sha256: null
  implementation:
...
Schema 4 adds complete, authoritative SFT training/checkpoint lineage to schema 3's intended-use,
therapeutic-quality, design-scope, lifecycle, viable-reference, and host-model contracts. Preserve
older artifacts as historical evidence; a new adapted project must resolve schema 4 rather than
interpreting missing fields as approval or not-applicable.

`sft_lineage` is required. It may be populated directly or by an immutable manifest reference whose
hash is verified and whose contents satisfy every field in the block. A checkpoint path, the general
`evidence` list, or another unconstrained mapping cannot substitute for it. Bind the checkpoint
identity/hash to the named stage and verify portable identities plus locally resolved paths before
implementation, calibration, or RL. Keep prompt and validation-generation lineage separate because
they describe sampling inputs, not SFT training/checkpoint provenance.
...
For every learned model whose output affects an online reward, checkpoint comparison, or candidate
filter or selection decision, the evidence table or model-specific block also records its downstream
role, expected generated-candidate envelope, required generalization claim, split axes and grouping,
confounding audit, standalone and incremental system metrics, applicability limits, and unsupported or
out-of-domain behavior. The contract is incomplete when these fields do not justify the model's actual
use. Generalization beyond the stated use is optional; evidence within that use is not.
