...
## Learned predictors inside an RL decision system

Before choosing a split, define every decision influenced by the model output and the domain and range
of candidates the RL system is expected to generate. Include the generator and checkpoint family,
prompt and sampling regime, relevant sequence or entity range, constraints imposed by other objectives,
and expected sources of deployment shift. Match holdout axes and performance claims to that envelope:
do not require transport to populations the system will not use, and do not accept random or
near-duplicate splits when expected use crosses lineage, study, site, time, assay, or batch boundaries.
If the available data cannot support the required claim, narrow the model's role or collect suitable
evidence.

Audit dataset lineage before fitting: biological grouping, duplicates, study/site/time/assay,
collection mode, batch, cohort composition, and their association with labels. When source and outcome
are entangled—for example, retrospective and prospective cohorts collected under different
conditions—use appropriate grouping, matching, stratification, mixed-source lockboxes, or prospective
bridge data. Report source- and subgroup-stratified performance, calibration, uncertainty, and tests
for shortcut learning.

Evaluate the predictor on its own and for incremental value in the complete reward and filter system.
Other independently validated objectives may legitimately restrict deployment or cover complementary
constraints, so a component model need not be universally valid in isolation. Demonstrate that
coverage with ablations, counterexamples, and end-to-end decision metrics rather than assuming it.
Record applicability limits and unsupported or out-of-domain handling, and revalidate after a material
change to the generation envelope or the model's decision role.
