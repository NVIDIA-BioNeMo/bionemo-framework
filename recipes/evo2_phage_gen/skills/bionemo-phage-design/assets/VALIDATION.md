...
| Codex plugin bridge         | PASS — the official validator accepts `recipes/evo2_phage_gen/.codex-plugin/plugin.json` and its `./skills/` bundle                                                                           |
...
```bash
PHAGE_SKILL_ROOT="${PHAGE_SKILL_ROOT:-$PWD/recipes/evo2_phage_gen/skills}"
...
Normal local recipe pytest runs
[`test_skill_evals.py`](../../../tests/bionemo/evo2_phage_gen/test_skill_evals.py), which invokes
the runner with `--dry-run --all`. This discovers and prepares every case without model calls. The
test is skipped when `CI` is set; live behavioral evals remain explicit commands.
...
```bash
python recipes/evo2_phage_gen/skills/bionemo-phage-design/scripts/tests/test_run_skill_evals.py
```
