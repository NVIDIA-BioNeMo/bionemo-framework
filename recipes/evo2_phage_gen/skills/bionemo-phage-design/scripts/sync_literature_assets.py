...
PAPERS: dict[str, PaperSpec] = {
    ...
    "black-2026-design-efficiency": PaperSpec(
        ...
        equation_tex=(
            r"\left(1 - p_{\mathrm{eff}}\right)^k = 0.053",
            r"\left(1 - p_{\mathrm{eff}}\right)^k = V_{\mathrm{observed}} \tag{1}",
            r"\operatorname{Uplift}(k) = \frac{\Pr_{\mathrm{model}}(\mathrm{viable}\mid k)}{\Pr_{\mathrm{base}}(\mathrm{viable}\mid k)} \tag{2}",
            r"\operatorname{Uplift}(k) = \frac{V_{\mathrm{observed}}}{(1-p_{\mathrm{lethal}})^k} \tag{3}",
            r"w = \exp(-s_{\mathrm{del}}k_d + s_{\mathrm{ben}}k_b) \tag{4}",
            r"d = \frac{k}{L}. \tag{5}",
            r"ID(d) = \frac14 + \frac34 e^{-4d/3}. \tag{6}",
            r"B \in \{5,20,100,1000\}. \tag{7}",
            r"g \in \{3,5,10\}. \tag{8}",
            r"\mu \in \{3\times10^{-7},10^{-6},3\times10^{-6}\}. \tag{9}",
            r"p_{\mathrm{del}} \in \{0.3,0.5,0.7\}, \tag{10}",
            r"p_{\mathrm{neu}} = 1-p_{\mathrm{del}}-p_{\mathrm{ben}}. \tag{11}",
            r"s_{\mathrm{del}} \in \{0.1,0.2,0.5\},\quad s_{\mathrm{ben}} \in \{0.05,0.1,0.2\}. \tag{12}",
        ),
    ),
}
...
_SPLIT_EXTERNAL_URL = re.compile(
    r"\[(https?://)([^\]\s]+)\]\(\1\2\)\s+\[([^\]\s]+)\]\(https?://\3\)(/?)",
    re.IGNORECASE,
)
_SPLIT_LINK_TEXT_URL = re.compile(
    r"\[(https?://[^\]\s]+)\]\(\1\)\s+((?=[A-Za-z0-9._~:/?#@!$&'*+=%-]*[./])[A-Za-z0-9._~:/?#@!$&'*+=%-]+)",
    re.IGNORECASE,
)


def _coalesce_split_external_urls(markdown: str) -> str:
    """Repair publisher markup that splits one visible URL across two links."""

    def replace(match: re.Match[str]) -> str:
        url = "".join(match.groups())
        return f"[{url}]({url})"

    def replace_plain_suffix(match: re.Match[str]) -> str:
        suffix = match.group(2)
        trailing = ""
        while suffix.endswith((".", ",", ";", ":")):
            trailing = suffix[-1] + trailing
            suffix = suffix[:-1]
        url = match.group(1) + suffix
        return f"[{url}]({url}){trailing}"

    markdown = _SPLIT_EXTERNAL_URL.sub(replace, markdown)
    return _SPLIT_LINK_TEXT_URL.sub(replace_plain_suffix, markdown)
...
class _MarkdownRenderer:
    ...
    def nodes(self, nodes: Sequence[Node | str]) -> str:
        ...
        rendered = _coalesce_split_external_urls(rendered)
        rendered = re.sub(r"[ \t]+\n", "\n", rendered)
        rendered = re.sub(r"\n{3,}", "\n\n", rendered)
    ...
    def node(self, value: Node | str) -> str:
        ...
        if id(node) in self.figures:
            ...
            anchors = list(
                dict.fromkeys(_safe_anchor(item.attrs.get("id", "")) for item in node.walk() if item.attrs.get("id"))
            )
            anchor_markup = "\n".join(f'<a id="{anchor}"></a>' for anchor in anchors if anchor)
            images = "\n".join(
        ...
        if node.tag == "img":
            ...
            return (equation.markdown or f"![{equation.label}]({equation.output_path})") if equation else ""
        ...
        if node.tag == "br":
            return "  \n"
        if node.tag == "a":
            label = self.children(node).strip() or node.attrs.get("title", "")
            href = unescape(node.attrs.get("href", ""))
            anchor = _safe_anchor(node.attrs.get("id", "") or node.attrs.get("name", ""))
        ...
        if node.tag == "li":
            anchor = _safe_anchor(node.attrs.get("id", ""))
            prefix = f'<a id="{anchor}"></a>\n' if anchor else ""
            return f"\n{prefix}- {self.children(node).strip()}"
...
def convert_article(source: str, spec: PaperSpec) -> ConvertedArticle:
    ...
    equations: dict[int, MediaItem] = {}
    equations_by_url: dict[str, MediaItem] = {}
    for node in article.walk():
        if node.tag != "img" or id(node) in figure_descendants:
            continue
        source_url = node.attrs.get("data-src") or node.attrs.get("src", "")
        if source_url.startswith("data:") or not re.search(r"/graphic-\d+\.gif(?:\?|$)", source_url, re.IGNORECASE):
            continue
        source_url = _canonical_media_url(_absolute_url(spec.article_url, source_url))
        item = equations_by_url.get(source_url)
        if item is None:
            number = len(equations_by_url) + 1
            markdown = None
            if spec.equation_tex:
                if number > len(spec.equation_tex):
                    raise SourceValidationError(
                        f"{spec.slug}: configured TeX count is smaller than the discovered equation count"
                    )
                markdown = f"$$\n{spec.equation_tex[number - 1]}\n$$"
            item = MediaItem(
                source_url,
            ...
            equations_by_url[source_url] = item
            media.append(item)
        equations[id(node)] = item
    if spec.equation_tex and len(spec.equation_tex) != len(equations_by_url):
        raise SourceValidationError(
            f"{spec.slug}: configured TeX count {len(spec.equation_tex)} does not match "
            f"the discovered equation count {len(equations_by_url)}"
        )
...
class _AllowlistRedirectHandler(urllib.request.HTTPRedirectHandler):
    """Reject a redirect target before urllib sends the next request."""

    def redirect_request(self, req, fp, code, msg, headers, newurl):
        """Validate one redirect hop before delegating request construction."""
        target = urllib.parse.urljoin(req.full_url, newurl)
        validate_redirect_chain(req.full_url, target)
...
class Fetcher:
    ...
    def _curl_download(self, url: str) -> bytes:
        """Use curl while validating every redirect before the next request."""
        executable = shutil.which("curl")
        if executable is None:
            raise RetryableFetchError("publisher retries exhausted and curl is unavailable")
        validate_redirect_chain(url, url)
        ...
        os.close(descriptor)
        header_descriptor, header_name = tempfile.mkstemp(prefix="curl-", suffix=".headers", dir=self.cache.root)
        ...
        try:
            for redirect_count in range(6):
                ...
                if len(fields) != 3 or not fields[0].isdigit():
                    raise RetryableFetchError(f"curl returned invalid response metadata for {current_url}")
                status, final_url, content_type = int(fields[0]), fields[1], fields[2]
                validate_redirect_chain(current_url, final_url)
                if status in {301, 302, 303, 307, 308}:
                    locations = [
                        line.partition(":")[2].strip()
                        for line in header_path.read_text(encoding="iso-8859-1").splitlines()
                        if line.partition(":")[0].strip().casefold() == "location"
                    ]
                    if not locations:
                        raise SourceValidationError(f"HTTP {status} response has no Location header: {current_url}")
                    if redirect_count == 5:
                        raise SourceValidationError(f"curl redirect limit exceeded for {url}")
                    target = urllib.parse.urljoin(final_url, locations[-1])
                    validate_redirect_chain(final_url, target)
                    current_url = target
                    continue
                if status in {403, 429} or status >= 500:
                    raise RetryableFetchError(f"HTTP {status} for {current_url}")
                if status >= 300:
                    raise HttpFetchError(status, current_url)
...
def _markdown_anchors(source: str) -> set[str]:
    """Return explicit HTML anchors and GitHub-style heading anchors."""
    anchors = {
        _safe_anchor(match.group(1))
        for match in re.finditer(r"<a\s+[^>]*(?:id|name)=[\"']([^\"']+)[\"'][^>]*>", source, re.IGNORECASE)
    }
    anchors.update(
        _safe_anchor(match.group(1)) for match in re.finditer(r"^#{1,6}\s+(.+?)\s*#*\s*$", source, re.MULTILINE)
    )
...
def check_asset_tree(root: Path) -> list[str]:
    ...
    for relative, path in actual.items():
        ...
        for match in _MARKDOWN_LINK.finditer(source):
            ...
            resolved = path.resolve() if not target else (path.parent / urllib.parse.unquote(target)).resolve()
            try:
                resolved.relative_to(root.resolve())
            except ValueError:
                errors.append(f"local link escapes asset root: {relative}: {raw_target}")
                continue
            ...
            if separator:
                try:
                    ...
                    errors.append(f"broken local fragment: {relative}: {raw_target}")
                    continue
                anchor = _safe_anchor(urllib.parse.unquote(fragment))
                if not anchor or anchor not in _markdown_anchors(destination_source):
                    errors.append(f"broken local fragment: {relative}: {raw_target}")
    return errors
