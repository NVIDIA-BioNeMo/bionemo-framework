...
This controller runs only from the recipe-local package that exposed it. Derive `recipe_root` from that package: it must be the `recipes/evo2_phage_gen` directory containing this controller at `skills/bionemo-phage-design/SKILL.md`. Do not use an unrelated or external skill root.

Validate only the colocated markers needed to guard a corrupt invocation: the recipe's `pyproject.toml`, `skills/bionemo-phage-design/SKILL.md`, and the local Codex and Claude plugin manifests. If any marker is missing, stop, state the exact observed path and missing marker, and direct the user back to `bionemo-phage-generation`.
