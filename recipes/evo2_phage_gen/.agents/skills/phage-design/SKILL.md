...
4. Choose case-study-replication or adapted-design. Select a concrete bacterial phage by default, or an archaeal virus when explicitly requested; record reference genome/hash, prokaryotic host, related genome collection, and intended outcome.
...
Scope work to viruses of bacteria or archaea. If asked to optimize eukaryotic entry, infection, replication, tropism, or host range—directly or by proxy—state that it is scientifically unsupported or likely implausible here and outside scope, then refuse that objective. Do not surface this boundary otherwise.
