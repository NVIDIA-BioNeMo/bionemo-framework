...
def _meta_values(root: Node, name: str) -> list[str]:
    return [node.attrs.get("content", "") for node in root.walk() if node.tag == "meta" and node.attrs.get("name", "").lower() == name.lower()]


def _contains_heading(node: Node, wanted: str) -> bool:
    wanted = wanted.casefold()
    return any(child.tag in {"h1", "h2", "h3"} and " ".join(child.text().split()).casefold() == wanted for child in node.walk())
...
def _validate_article_identity(root: Node, spec: PaperSpec) -> tuple[str, tuple[str, ...]]:
    ...
    licenses = [node.attrs.get("href", "") for node in root.walk() if node.tag == "link" and "license" in node.attrs.get("rel", "").split()]
...
class _MarkdownRenderer:
    _BLOCKS = {"div", "section", "p", "ul", "ol", "li", "table", "tr", "blockquote"}
    ...
    def node(self, value: Node | str) -> str:
        ...
        if id(node) in self.figures:
            ...
            images = "\n".join(f"![{label}{f', part {index}' if len(items) > 1 else ''}]({item.output_path})" for index, item in enumerate(items, 1))
        ...
        if node.tag == "table":
            ...
            for row in node.walk():
                if row.tag != "tr":
                    continue
                cells = [" ".join(cell.text().split()) for cell in row.children if isinstance(cell, Node) and cell.tag in {"th", "td"}]
                if cells:
                    rows.append(cells)
            if not rows:
                return f"\n{self.children(node)}\n"
...
def convert_article(source: str, spec: PaperSpec) -> ConvertedArticle:
    ...
    supplement_index = next(
        (index for index, child in enumerate(article.children) if isinstance(child, Node) and _contains_heading(child, "Supplementary material")),
        None,
    )
    ...
    if supplement_index is None:
        ...
        acknowledgement_index = next(
            (
                ...
                if isinstance(child, Node)
                and ("ack" in child.classes or _contains_heading(child, "Acknowledgements") or _contains_heading(child, "Acknowledgments"))
            ),
            None,
        )
    ...
    main_ids = {node.attrs["id"] for value in main_nodes if isinstance(value, Node) for node in value.walk() if node.attrs.get("id")}
    supplement_ids = {node.attrs["id"] for value in supplement_nodes if isinstance(value, Node) for node in value.walk() if node.attrs.get("id")}
    common = dict(
...
def is_allowed_url(url: str) -> bool:
    ...
    return parts.scheme == "https" and (parts.hostname or "").casefold() in ALLOWED_HOSTS and not parts.username and not parts.password
...
def validate_redirect_chain(initial_url: str, final_url: str) -> None:
    if not is_allowed_url(initial_url) or not is_allowed_url(final_url):
        raise SourceValidationError(f"URL or redirect target is outside the HTTPS host allowlist: {initial_url!r} -> {final_url!r}")
...
def read_xlsx_sheet(workbook: Path, sheet_name: str) -> XlsxSheet:
    ...
    with archive:
        ...
        targets = {item.attrib.get("Id"): item.attrib.get("Target") for item in rel_root.findall(f"{{{_PKG_REL_NS}}}Relationship")}
...
def _sheet1_tsv(workbook: Path, *, expected_rows: int, expected_columns: int) -> tuple[bytes, dict[str, object]]:
    ...
    if len(sheet.rows) != expected_rows + 1:
        raise WorkbookValidationError(f"Sheet 1 expected {expected_rows} data rows, found {max(0, len(sheet.rows) - 1)}")
...
def _validate_staged(root: Path, spec: PaperSpec) -> None:
    ...
    if len(figures) != spec.expected_figure_files:
        raise SourceValidationError(f"{spec.slug}: expected {spec.expected_figure_files} figure files, found {len(figures)}")
    if len(equations) != spec.expected_equations:
        raise SourceValidationError(f"{spec.slug}: expected {spec.expected_equations} equation files, found {len(equations)}")
    ...
    try:
        ...
        if spec.workbook_url:
            ...
            try:
                ...
                workbook_source = "explicit-local-fallback:dist/media-1.xlsx"
...
def _parser() -> argparse.ArgumentParser:
    ...
    sync_parser = subparsers.add_parser("sync", help="download, convert, validate, and atomically install literature assets")
    sync_parser.add_argument("--paper", action="append", choices=sorted(PAPERS), help="paper slug; repeat; default: all")
    ...
    check_parser.add_argument("--paper", action="append", choices=sorted(PAPERS), help="paper slug; repeat; default: all")
