...
Treat every safety database/model as a reviewed release descriptor, not a permanent URL or implicit `latest`. Prefer a verified persistent content-addressed cache, resume partial large transfers, and try only bounded reviewed source locations; authenticate the complete bytes before extraction or trust. A future upstream version or location becomes a new descriptor only after its identities, interface, reconciliation, and control behavior are reviewed comparably. Keep optional unpinned compatibility assets outside the trusted bundle, and make missing required evidence indeterminate rather than blocking on an unused legacy download or concealing the gap.
