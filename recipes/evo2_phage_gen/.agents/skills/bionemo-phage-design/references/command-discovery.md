...
For PHROGs v4, use the Pharokka v1.8.0 Zenodo record 17110353 as the default failover when QC needs an MMseqs search interface. Verify the Zenodo-published checksum, archive structure, transformation, and an MMseqs open/search smoke test. Record that it is derived rather than an exact `FAA_phrog.tar.gz` mirror, and do not pass it to a consumer that requires the raw FAA sequences.
