...
Resolve commands from the checkout and active environment at execution time. Skill examples and old run logs are snapshots, never authority.
...
1. Locate the recipe and read current pyproject.toml, especially [project.scripts].
2. Build and activate the documented environment. For the current phage recipe, discover and run .ci_build.sh before sourcing .ci_test_env.sh; verify names in the checkout.
3. Run the selected entry point's --help and, when supported, --version. Resolve it with command -v.
4. Inspect the mapped source function, nearest maintained config, focused tests, and current README. Follow repository symlinks deliberately.
...
2. search .agents/skills/** for the old interface;
