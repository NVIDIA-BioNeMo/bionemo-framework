...
3. Verify `planning/DESIGN_SPEC.yaml`, RL_OBJECTIVES.yaml, fixed validation-generation manifest, filter profile, policy/KL identities, runtime contract, hardware/disk, and execution plan. Apply the central [GPU training, RL, and generation policy](../bionemo-phage-design-adapt-execution/references/resource-and-oom-policy.md#gpu-training-rl-and-generation). For adapted therapeutic work, require the resolved intended-use applicability block and enabled design-relevant therapeutic-quality components. For the current PhiX174 replication default, require its approved customized profile—filters 1–6, 8, and 9 enabled, filter 7 disabled, and applicable safety components enabled by default—with historical and added component-set identities kept separate. Consume this planning decision without reinterpretation. Reject a whole-genome project whose resolved generation or scoring path silently restricts mutation or evaluation to a locus/module. Resolve commands with [command resolution](references/command-resolution.md) and record them through [action traceability](references/action-traceability.md).
