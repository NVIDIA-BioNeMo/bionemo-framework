...
- Monitor global steps (distinguish per-epoch banners), logs/config/checkpoints/source/free space, and process/scheduler state. Verify watchdog heartbeats after launch; a running worker alone does not prove supervision. Keep local TensorBoard authoritative; prefer authenticated W&B in an RL-specific project sharing the project-family prefix, and fall back without blocking. Follow [the monitoring contract](references/monitoring-contract.md) for due-gated cadence, comparability, stopping, and rebound.
