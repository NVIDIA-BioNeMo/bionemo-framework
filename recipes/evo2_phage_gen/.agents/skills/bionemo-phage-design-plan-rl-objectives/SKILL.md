...
while excluding downstream manufacturing controls from sequence rewards. Keep the historical score
for base-paper replication unless the user requests these additions. For an explicitly
non-therapeutic project, record the intended-use rationale and the reason for every omitted
therapy-specific item.
