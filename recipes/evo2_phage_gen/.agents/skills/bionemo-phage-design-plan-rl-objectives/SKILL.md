...
1. Record reference phage/hash, target host, desired change, protected traits, ordering budget, acceptable uncertainty, and proposed prompt semantics. Honor fresh/no-reuse instructions. Otherwise use an approved SFT choice or ask only when candidates differ materially.
