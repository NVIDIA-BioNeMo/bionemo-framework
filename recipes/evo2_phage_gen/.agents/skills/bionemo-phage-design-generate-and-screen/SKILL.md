...
4. Run the approved nested hard all/any tree over complete candidates. Recheck whole-genome structure, lifecycle-wide viability signals, harmful-cargo and other [therapeutic-suitability exclusion rules](../bionemo-phage-design/references/ema-2025-draft-phage-therapy-quality-guideline.md) when applicable, and host-range direction independently of any aggregate model score. Keep whole-sequence cargo and lysogeny screens, evidence for strictly lytic behavior, and lifecycle/transduction endpoints separate; unresolved experimental evidence remains indeterminate even when computational screens pass. Apply the same source-neutral sequence criteria to reference, database, metagenomic, local, and model-generated candidates: provenance is required for traceability, but natural origin is not a hard filter. Mandatory missing/failure fails closed. Record waterfalls, OR overlap, and dominance.
...
policy, tool, and parser hashes. Benchmark a representative pilot, then set bounded record workers and
per-tool threads so their product fits allocated CPU plus measured RAM, storage-I/O, and scratch limits;
avoid nested oversubscription and preserve deterministic input/output mapping. Use a GPU path only when
the pinned tool supports the required query/target orientation and database layout, it materially helps
at the intended batch size, and CPU/GPU outputs plus the complete reference-control panel agree.
