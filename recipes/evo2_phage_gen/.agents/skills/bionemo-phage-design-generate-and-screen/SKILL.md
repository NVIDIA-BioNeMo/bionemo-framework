...
1. Freeze checkpoint, rollout manifest/seeds, objective/filter versions, tools/models, and source/config hashes. Inherit the exact deployed RL prompt strata, mixture weights, and sampling values; use independent IDs/seeds and audit overlap with calibration/RL validation. Any scientific deviation requires explicit rationale and fresh comparable calibration. Derive and behavior-test the exact runtime suite required by enabled filters. Before candidate PASS decisions, run the same binaries, assets, policies, and parsers against `configs/phage_safety_reference_controls.yaml` and validate the complete panel with `evo2_phage_validate_safety_controls`: CTXphi `NC_015209.1` must independently fail lysogeny/integration and detect ctxA/P01555 plus ctxB/P01556; T7 `NC_001604.1`, phiX174 `NC_001422.1`, and Muddy `NC_022054.2` must produce their measured negative-control PASS outcomes. Any missing, indeterminate, identity-drifted, false-negative, or false-positive control blocks candidate PASS promotion. This panel tests the deployed software contract, not biological, therapeutic, clinical, or regulatory validity. Make unattended stage chains single-instance, atomically stateful, tolerant of transient reads, heartbeat-visible, bounded-retry, and resumable.
...
CPU/GPU outputs plus the complete reference-control panel agree.
