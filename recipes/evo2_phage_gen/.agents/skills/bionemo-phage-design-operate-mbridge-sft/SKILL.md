...
2. Invoke bionemo-phage-design-adapt-execution; use its environment, storage, ordered-script, monitoring, resume, and centralized resource and OOM contracts. On the actual SFT execution host run `./.ci_build.sh` before `source .ci_test_env.sh`, then resolve and verify current entry points and imports from pyproject.toml, --help, source, nearest config and tests, and README. Never launch from a chat-only, historical, or differently built environment.
