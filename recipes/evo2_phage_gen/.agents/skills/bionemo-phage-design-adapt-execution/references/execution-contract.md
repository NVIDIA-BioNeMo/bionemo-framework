...
Treat `../../ci/lepton/` from the recipe root as evidence of a Hydra job path and pinned client, not a universal launcher. Discover authentication, workspace/endpoint, API, node group/resources, image/registry, mounts, secret references, egress, naming, status/log/cancel APIs, and durable-output ownership. Never copy site-specific defaults blindly.

Build the recipe with .ci_build.sh, then source .ci_test_env.sh before resolving entry points. Emit project-owned config and submit/status/log/cancel/resume wrappers. Pin revision/container digest. Leave unverified endpoint/image/auth variants unresolved.
