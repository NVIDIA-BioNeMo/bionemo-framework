...
Write the collection FASTA(s), metadata table, exclusions, query log, source manifest, and checksums beneath `artifacts/`; finish `OUTPUTS.yaml`, `SUMMARY.md`, and `RUNLOG.md`. Include exact paths and hashes for `bionemo-phage-design-prepare-sft`, but do not update root stable pointers yourself.
