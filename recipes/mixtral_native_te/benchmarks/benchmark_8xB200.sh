...
OUTPUT_DIR="${OUTPUT_DIR:-${TMPDIR:-/tmp}/mixtral_native_te_8xB200}"
...
TOKEN_MICRO_BATCH_SIZE="${TOKEN_MICRO_BATCH_SIZE:-4096}"
FP8_PEAK_PFLOPS="${FP8_PEAK_PFLOPS:-4.5}"
BF16_PEAK_PFLOPS="${BF16_PEAK_PFLOPS:-2.25}"
...
OUTPUT_DIR="$OUTPUT_DIR" PRECISIONS="$PRECISIONS" PARALLEL_CONFIGS="$PARALLEL_CONFIGS" \
    FP8_PEAK_PFLOPS="$FP8_PEAK_PFLOPS" BF16_PEAK_PFLOPS="$BF16_PEAK_PFLOPS" python - <<'PY'
import os
import re
from pathlib import Path
...
print("dp,ep,precision,tokens_per_s_per_gpu,pflops_per_gpu,mfu_pct,step_time_s,mem_gb,last_loss,n_steady,elapsed_s,rc")
